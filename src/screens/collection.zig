...
const CollectionColumnContext = struct {
    state: *const State,
    accent: chasen.Color,
};
...
pub const State = struct {
    ...
    fn drawCollectionColumnList(self: *const State, list: *const ui.List, surface: *chasen.Surface, opts: ViewOptions) !void {
        const column_context: CollectionColumnContext = .{
            .state = self,
            .accent = opts.accent,
        };
        try column_list_view.viewVisibleRows(list, surface, &collection_columns, &column_context, buildCollectionColumnRow, .{
...
fn buildCollectionColumnRow(context: *const anyopaque, allocator: std.mem.Allocator, visible_index: usize) !ui.ColumnList.Row {
    const column_context: *const CollectionColumnContext = @ptrCast(@alignCast(context));
    const state = column_context.state;
    const source_index = state.sourceIndex(visible_index) orelse return &.{};
    return collectionColumnRow(allocator, state.items[source_index], column_context.accent);
}

fn collectionColumnRow(allocator: std.mem.Allocator, item: bgg_model.CollectionItem, accent: chasen.Color) !ui.ColumnList.Row {
    const row = try allocator.alloc(ui.ColumnList.Cell, collection_columns.len);
    row[0] = .{ .text = try collectionItemName(allocator, item) };
    row[1] = .{ .text = "♥", .style = .{ .fg = accent } };
    row[2] = .{ .text = try collectionRatingText(allocator, item.rating) };
    row[3] = .{ .text = "★", .style = .{ .fg = accent } };
    row[4] = .{ .text = try collectionRatingText(allocator, item.bgg_rating) };
    row[5] = .{ .text = try collectionRankText(allocator, item.rank) };
    row[6] = .{ .text = try collectionPlaysText(allocator, item.num_plays) };
    return row;
}
...
fn collectionRatingText(allocator: std.mem.Allocator, rating: f64) ![]const u8 {
    if (rating <= 0) return "-";
