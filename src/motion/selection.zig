...
fn glitchShouldReplace(frame: u64, index: u32, bytes: []const u8) bool {
    ...
    const tick = frame / 4;
    return common.glitchHash(tick, index) % 100 < 8;
}

fn glitchReplacement(frame: u64, index: u32) []const u8 {
    const tick = frame / 4;
    return common.glitch_chars[common.glitchHash(tick + 17, index) % common.glitch_chars.len];
}
